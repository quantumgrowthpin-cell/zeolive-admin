...
import { canViewModule, isSubAdminUser } from '@/util/permissions'

// Transform the data to group items by their sections
const getFilteredSearchData = () => {
  if (!isSubAdminUser()) return data; // Admin → full access
  // return data?.filter(item => canViewModule(item.section));
  const { permissions } = JSON.parse(sessionStorage.getItem("subadmin") || "[]");
  const allowedSections = permissions.filter(p => p.canView).map((p) => p.section.toLowerCase());

  
return data?.filter((item) => allowedSections.includes(item.name.toLowerCase()) || allowedSections.includes(item.section.toLowerCase()))
};

const rawData = getFilteredSearchData();
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          {searchValue ? (
            ...
            !isSubAdminUser() && <DefaultSuggestions setOpen={setOpen} />
          )}
        </CommandList>
        <CommandFooter />
      </CommandDialog>
